...
trait CurrentEventsBySliceQuery extends ReadJournal {
  ...
  /**
   * Same type of query as [[pekko.persistence.query.typed.javadsl.EventsBySliceQuery.eventsBySlices EventsBySliceQuery.eventsBySlices]] but the event stream is completed immediately when it
