...
object SupervisorStrategy {
  ...
  /**
   * Restart immediately without any limit on number of restart retries. A limit can be added with
   * [[RestartSupervisorStrategy.withLimit(maxNrOfRetries:Int,withinTimeRange:java\.time\.Duration)* RestartSupervisorStrategy.withLimit]].
   *
   * If the actor behavior is deferred and throws an exception on startup the actor is stopped
   * (restarting would be dangerous as it could lead to an infinite restart-loop)
   */
...
sealed abstract class BackoffSupervisorStrategy extends SupervisorStrategy {
   ...
   *
   * The counter (and log level) is reset after the
   * [[BackoffSupervisorStrategy.withResetBackoffAfter(timeout:java\.time\.Duration)* BackoffSupervisorStrategy.withResetBackoffAfter]]
   * duration.
   */
