...
/**
 * Marker supertype for a setup part that can be put inside [[pekko.actor.setup.ActorSystemSetup ActorSystemSetup]], if a specific concrete setup
 * is not specified in the actor system setup that means defaults are used (usually from the config file) - no concrete
 * setup instance should be mandatory in the [[pekko.actor.setup.ActorSystemSetup ActorSystemSetup]] that an actor system is created with.
 */
abstract class Setup {

  /**
   * Construct an [[pekko.actor.setup.ActorSystemSetup ActorSystemSetup]] with this setup combined with another one. Allows for
...
object ActorSystemSetup {
  ...
  /**
   * Scala API: Create an [[pekko.actor.setup.ActorSystemSetup ActorSystemSetup]] containing all the provided settings
   */
  def apply(settings: Setup*): ActorSystemSetup =
    new ActorSystemSetup(settings.map(s => s.getClass -> s).toMap)

  /**
   * Java API: Create an [[pekko.actor.setup.ActorSystemSetup ActorSystemSetup]] containing all the provided settings
   */
  @varargs
